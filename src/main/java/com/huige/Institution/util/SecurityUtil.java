package com.huige.Institution.util;

import com.huige.Institution.constant.RespCode;
import com.huige.Institution.domain.vo.LoggedInUser;
import com.huige.Institution.exception.CustomException;
import org.springframework.security.core.Authentication;
import org.springframework.security.core.context.SecurityContextHolder;
import org.springframework.security.crypto.bcrypt.BCryptPasswordEncoder;


/**
 * 安全服务工具类
 *
 * @author hying
 */
public class SecurityUtil {
    /**
     * 获取登录用户名
     **/
    public static String getUsername() {
        try {
            return getLoggedInUser().getUsername();
        } catch (Exception e) {
            throw new CustomException("获取用户名异常", RespCode.UNAUTHORIZED);
        }
    }

    /**
     * 获取登录用户id
     **/
    public static Integer getUserId() {
        try {
            return getLoggedInUser().getUserId();
        } catch (Exception e) {
            throw new CustomException("获取用户id异常", RespCode.UNAUTHORIZED);
        }
    }

    /**
     * 获取用户
     **/
    public static LoggedInUser getLoggedInUser() {
        try {
            return (LoggedInUser) getAuthentication().getPrincipal();
        } catch (Exception e) {
            throw new CustomException("获取用户信息异常", RespCode.UNAUTHORIZED);
        }
    }

    /**
     * 获取Authentication
     */
    public static Authentication getAuthentication() {
        return SecurityContextHolder.getContext().getAuthentication();
    }

    /**
     * 生成BCryptPasswordEncoder密码
     *
     * @param password 密码
     * @return 加密字符串
     */
    public static String encryptPassword(String password) {
        BCryptPasswordEncoder passwordEncoder = new BCryptPasswordEncoder();
        return passwordEncoder.encode(password);
    }

    /**
     * 判断密码是否相同
     *
     * @param rawPassword     真实密码
     * @param encodedPassword 加密后字符
     * @return 结果
     */
    public static boolean matchesPassword(String rawPassword, String encodedPassword) {
        BCryptPasswordEncoder passwordEncoder = new BCryptPasswordEncoder();
        return passwordEncoder.matches(rawPassword, encodedPassword);
    }

}
